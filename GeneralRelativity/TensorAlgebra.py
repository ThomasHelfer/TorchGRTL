...
def compute_christoffel(d1_metric, h_UU):
    """
    ...
    shape = d1_metric.shape[:-1] + (d1_metric.shape[-2],)  # shape is [batch, x, y, z, i, j, k]
    ...
    for i in range(3):
        for j in range(3):
            for k in range(3):
                LLL[..., i, j, k] = 0.5 * (
                    + d1_metric[..., j, i, k] + d1_metric[..., k, i, j] 
                    -d1_metric[..., j, k, i])
...
def compute_christoffel_fast(d1_metric, h_UU):
    """
    Computes the Christoffel symbols of the first kind (LLL) and second kind (ULL) 
    ...
    LLL = 0.5 * (test.permute(0,1, 2, 3, 4, 5, 6 ) + test.permute(0,1, 2, 3, 4, 6, 5 )  - 
