...
class TensorDict:
    def __init__(self, tensor, keys):
        ...
        if tensor.shape[self.num_index] != len(keys):
            raise ValueError(f"The number of keys must match the size of the tensor's {self.num_index} dimension.")
        self.tensor = tensor
        self.keys = keys
    ...
    def __getitem__(self, key):
        ...
        if key == 'h' or key == 'A':
            h_tensor = torch.zeros(self.tensor.shape[:self.num_index] + (3, 3) + self.tensor.shape[(self.num_index+1):] , dtype=self.tensor.dtype)
            for i in range(3):
                for j in range(i, 3):
                    ...
                    if ij_key in self.key_to_index:
                        ...
                        if (len(self.tensor.shape[(self.num_index+1):])==2):
                            for k in range(3):
                                for l in range(3):
                                    ...
                                    h_tensor[..., i, j,k,l] = h_tensor[..., j, i,k,l] = \
                                    self.tensor.index_select(self.num_index, torch.tensor([self.key_to_index[ij_key]]))[...,0,k,l]
                        # This is for dealing with 1st derivatives (and making sure derivatives indices are last)
                            ...
                            for k in range(3):
                                # making sure that the tensor is symmetric in ij 
                                h_tensor[..., i, j,k] = h_tensor[..., j,i,k] = \
                                self.tensor.index_select(self.num_index, torch.tensor([self.key_to_index[ij_key]]))[...,0,k]
                        # reference term
                        else:
                            # making sure that the tensor is symmetric in ij 
                            h_tensor[..., i, j] = h_tensor[..., j, i] = self.tensor[..., self.key_to_index[ij_key]]
            return h_tensor
        ...
        # for 2nd derivative it returns d_x d_y v_i with shape [batch,xdim,ydim,zdim,i,x,y]
            ...
            return torch.index_select(self.tensor, self.num_index, torch.tensor(indices))
        ...
        # for 2nd derivative it returns d_x d_y swith shape [batch,xdim,ydim,zdim,x,y]
            ...
            return torch.index_select(self.tensor, self.num_index, torch.tensor([self.key_to_index[key]])).squeeze(self.num_index)
        else:
            raise KeyError(f"Key '{key}' not found.")

def get_binary_data(filename, num_variables):
    data_list = []
    length = 0
...
def get_many_binary_files(filenames,num_variables):
...
def get_many_binary_files_as_array(filenames,num_variables):
    FilenamesList = glob.glob(filenames)
    tmp = []
    print('Loading data')
    for i in tqdm(FilenamesList):
